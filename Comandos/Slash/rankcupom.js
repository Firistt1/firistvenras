...
const db = new JsonDatabase({ databasePath: "./databases/cupom.json" });
const perms = new JsonDatabase({ databasePath:"./databases/myJsonPerms.json" });


module.exports = {
  name: "rankcupom",
  description:"「📊」Exiba o rank de cupons",
  ...
  run: async (client, interaction, message, args) => {
    ...
    const grana = db.all().filter(i => i.data.usado).sort((a, b) => b.data.usado - a.data.descontado);
    
    if (grana.length < 1) return interaction.reply(`Não Usaram nenhum cupom`, message);
    ...
    const displayPage = () => {
      ...
      const formattedValues = pageItems.map((entry, index) => {
        ...
        return `${medalha} | **__${numRanking}°__** - Cupom - ${entry.ID}\n${emoji.card} | Descontado: **R$${Number(entry.data.descontado).toFixed(2)}** \n${emoji.raio} | desconto: **${Number(dbc.get(`${entry.ID}.desconto`))}%**\n ${emoji.abobora} | Usado: **${Number(entry.data.usado)}**`;
      }).join('\n\n');
