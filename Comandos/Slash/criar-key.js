    ...
    run: async(client, interaction) => {
      ...
      const row = new Discord.ActionRowBuilder()
        ...
        const embed = await interaction.reply({ embeds: [new Discord.EmbedBuilder()
          .setTitle(`${dbcv.get(`title`)} | Criação de Key`)
          .addFields({name:`${emoji.caixa} | Estoque:`,value: `Nenhum...`})
             ...
             collector.on("collect", message => {
               ...
               for (let i = 0; i != estoque.length; i++) {
                 ...
                 if(i + 1 === estoque.length) {
                   ...
                   msg.edit(`${emoji.sim} | Key \`${gerado}\` Criado com sucesso!`)
  .then((editedMessage) => {
    setTimeout(() => {
      editedMessage.delete().catch(console.error);
                   ...
                   const embednew = new Discord.EmbedBuilder()
                     .setTitle(`${dbcv.get(`title`)} | Criação de Key`)
                     .addFields({name:`${emoji.caixa} | Estoque:`, value:`${texto}`})
                     .addFields({name:`${emoji.presente} | Código:`,value: `${gerado}`})
